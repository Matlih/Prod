...
export default function App() {
  ...
  const displayStateColor = () => {
    if (phase === 'work') return 'text-blue-600 dark:text-blue-400';
    if (phase === 'break') return 'text-amber-500 dark:text-amber-400';
  ...
  return (
    <div className={`flex flex-col items-center justify-center w-full h-screen relative overflow-hidden transition-colors duration-700 ${displayBgColor()} ${displayStateColor()}`}>
      ...
      <div className="flex flex-col items-center justify-center w-full h-full select-none z-10 relative pointer-events-none transition-transform duration-300 active:scale-[0.98]">
        ...
        <div className="text-[4vmin] md:text-sm font-medium tracking-widest uppercase mb-[2vmin] md:mb-4 opacity-70">
          {phase === 'idle' ? 'Ready' : phase === 'paused' ? 'Paused' : phase === 'work' ? 'Deep Work' : 'Rest'}
        </div>
        
        <div className="text-[25vmin] font-light tracking-tighter tabular-nums transition-colors duration-700 drop-shadow-sm leading-none">
          {formatTime(timeLeft)}
        </div>
